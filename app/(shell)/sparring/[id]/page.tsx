...
export default async function SparringSessionPage({
  ...
  const photos = await prisma.media.findMany({
    where: { attachedType: "SPARRING_SESSION", attachedId: session.id, kind: "SPARRING_MEDIA" },
    ...
    <div className="space-y-6 pt-2">
      ...
      {(photos.length > 0 || isHost) && (
        <section className="space-y-3">
          ...
          {isHost && (
            <MediaUploader kind="SPARRING_MEDIA" attachedType="SPARRING_SESSION" attachedId={session.id} label="Add photo" />
          )}
        </section>
      )}
